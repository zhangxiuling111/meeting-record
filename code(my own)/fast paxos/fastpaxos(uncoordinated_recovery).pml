...
chan acceptorphase2b[ACCEPTORS] = [10] of {byte, byte, byte}

//uncoordinated recovery，在
...
inline lprepare(id, round){
	byte i;
	for(i : 0 .. (ACCEPTORS-2)){
		acceptorpre[i] ! id,round;                    //由coordinator向quorum中的acceptor发送prepare请求
	}
	i = 0;
}
...
inline anymessage(id, round){
	byte i;
	for(i : 0 .. (ACCEPTORS-2)){                   //只向quorum的acceptor发送any message（默认quorum的id为0,1,2 ）
...
inline proaccept(id, round, value){
	byte i;
	for(i : 0 .. (ACCEPTORS-2)){
		acceptoracc[i] ! id,round,value;                //由proposer直接向quorum中的acceptor（默认quorum的id为0,1,2 ）发送accept请求
	}
	i = 0;
}

inline phase2b(id, round, value){                   //每个acceptor都会把phase2b消息转发给quorum中的其他acceptor
	byte i;
	for(i : 0 .. (ACCEPTORS-2)){
		acceptorphase2b[i] ! id,round,value;
	}
	i = 0;
}
...
proctype coordinator(byte id){
	...
	byte pre[ACCEPTORS];
	byte k;
	byte newval;              //coordinator在冲突恢复中开始的新的一轮中选中的value
...
proctype acceptor(byte id){
	...
	byte rnd,coorid; 
	short val;
	byte newval;    //在冲突恢复中开始的新的一轮中选中的value  
	byte acccount[PROPOSERS];           //用来保存每个value的接收次数,最多有proposers个value
	byte accvalue[ACCEPTORS];   //用来判断是否已经接收过来自某个acceptor的信息,若同一个idde acceptor接受多次，则不用反复计数 
	byte acceptorscount = 0, accmax = 0;  //分别表示已经接收了value的acceptor的个数和被接受次数最多的value的次数，用来判断是否会发生冲突，因为如果所有的acceptor都已经接收了value，却没有任何一个value的接受次数达到quorum，则有冲突
	byte maxvalue; //用来记录被接受次数最多的value值，即accmax对应的value
	byte accid,accrnd,accval; //来自同一个quorum中其他acceptor的信息；
	do
	...
	::  (acceptoracc[id] ? [coorid,rnd,val] && !progress )->acceptoracc[id] ? coorid,rnd,val   //acceptor查看accept请求
	    if
	    ...
	    :: rnd > crnd ->atomic{             //冲突恢复中直接开始的crnd+1轮
	        	...
	        	hval = val;
	        	learnacc ! id, crnd, val;
	        	phase2b(id,crnd,val);           //acceptor 接收一个value后向quorum中所有其他的acceptor发动phase2b消息
	        	//coordinatoracc[coorid] ! id, crnd, val;
	    }
	    ...
	    //acceptor接收来自quorum中的其他acceptor的phase2b投票信息；
	    ::acceptorphase2b[id]?[accid,accrnd,accval] -> acceptorphase2b[id]?accid,accrnd,accval
	    	if 
	    	:: accmax < QUORUM && acceptorscount < QUORUM ->        //对acceptor接受的value及各value的接收次数进行计数
	    		if
	    		:: accrnd !=0 && accrnd == crnd ->
		    		if
		    		:: accvalue[accid] == 0 ->
		    ...
		    :: accmax < QUORUM && acceptorscount >= QUORUM ->      //quorum中所有的acceptor都已经接收了value，却没有任何一个value的接受次数达到quorum，则有冲突
		        //发生冲突，使用uncoordinated recovery,每个acceptor都把自己接受的crnd的phase2b message当做crnd+1的phase2a message,直接开始crnd+1 的phase 2b阶段
		    	atomic{
					crnd ++;
					byte k =0;
					do
					:: acccount[k] == 0 -> k++;
					:: acccount[k] != 0 -> break;         //所有acceptor都选择i已经提交的value中id号最小的proposer提交的value，防止新的冲突
					od
					newval = k+1;
					cval = newval;
	        		hrnd = crnd;
	        		hval = cval;
	        		learnacc ! id, crnd, newval;
	        		phase2b(id,crnd,newval);

				}
			:: accmax >= QUORUM ->         //acceptor对某个value的接受次数达到quorum
	    		atomic{
				learned ! crnd,accval;
			}
			fi
